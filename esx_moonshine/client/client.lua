...
AddEventHandler('esx_moonshine:hasEnteredMarker', function(zone)
	...
	if action[2] == 'Processing' then
		ESX.TriggerServerCallback('esx_moonshine:getInventoryItem', function(qtd)
...
RegisterNetEvent('esx_moonshine:hasExitedMarker')
AddEventHandler('esx_moonshine:hasExitedMarker', function(zone)
	CurrentAction = nil
	ESX.UI.Menu.CloseAll()
	TriggerServerEvent('esx_moonshine:stopHarvest')
	TriggerServerEvent('esx_moonshine:stopTransform')
	TriggerServerEvent('esx_moonshine:stopSell')
	ClearPedTasks(PlayerPedId())
end)

-- Drug effect
RegisterNetEvent('esx_moonshine:onUse')
AddEventHandler('esx_moonshine:onUse', function(drug)
				...
				if(GetDistanceBetweenCoords(coords, j.x, j.y, j.z, true) < Config.DrawDistance) then
					if i == 'Dealer' then
						if NPCs[k] == nil then
							...
							TaskStartScenarioInPlace(NPCs[k], "WORLD_HUMAN_DRINKING", 0, true);
						end
					else
...
Citizen.CreateThread(function()
	while true do
		...
		if isInMarker and not hasAlreadyEnteredMarker then
			...
			TriggerEvent('esx_moonshine:hasEnteredMarker', currentZone)
		end

		if not isInMarker and hasAlreadyEnteredMarker then
			hasAlreadyEnteredMarker = false
			TriggerEvent('esx_moonshine:hasExitedMarker', lastZone)
		end
	end
end)
		...
		if CurrentAction ~= nil then
			...
			if IsControlJustReleased(0, Keys['E']) then
				...
				elseif action[2] == "Field" then
					...
					TriggerServerEvent('esx_moonshine:startHarvest', action[1])
				...
				elseif action[2] == "Processing" then
					TriggerServerEvent('esx_moonshine:startTransform', action[1])
					TaskStartScenarioInPlace(playerPed, 'PROP_HUMAN_BBQ', 0, false)
					callPolice(cops.AlertCops, cops.Zones[action[2]].callPolice, cops.Zones[action[2]].callPoliceChance, PlayerCoords)
				elseif action[2] == "Dealer" then
					TriggerServerEvent('esx_moonshine:startSell', action[1])
					callPolice(cops.AlertCops, cops.Zones[action[2]].callPolice, cops.Zones[action[2]].callPoliceChance, PlayerCoords)
				else
					isInZone = false
				end
